  ...
  try {
    ...
    if (image) {
      ...
      if (!imageUploadRes.success) {
        return {success: false, msg: imageUploadRes.msg || "Tải ảnh thất bại"}
      }
...
const updateWalletForNewTransaction = async(
  ...
  try {
    ...
    if (!walletSnapshot.exists()) {
      ...
      return { success: false, msg: "Không tìm thấy ví" };
    }
    ...
    // console.log("lỗi cập nhật ví cho giao dịch mới", err);
    return { success: false, msg: err.message };
  }
}
...
const revertAndUpdateWallets = async(
  ...
  try {
    ...
    if (newTransactionType == "expense") {
      // nếu sửa income -> expense cùng ví, hoặc tăng số tiền expense mà ví không đủ
      if (
        ...
        return {
          success: false,
          msg: "Ví bạn chọn không đủ số dư",
        };
      }

      // nếu thêm expense vào ví mới nhưng ví không đủ tiền
    ...
    // refetch new wallet vì có thể đã vừa update
...
export const deleteTransaction = async (
  ...
  try {
    ...
    if (!transactionSnapshot.exists()) {
      return { success: false, msg: "Không tìm thấy giao dịch" };
    }
    ...
    // fetch wallet để update amount, totalIncome hoặc totalExpenses
    const walletSnapshot = await getDoc(doc(firestore, "wallets", walletId));
    const walletData = walletSnapshot.data() as WalletType;

    // check field cần update theo transaction type
    const updateType =
    ...
    // nếu là income và xóa xong ví âm tiền thì chặn
...
export const fetchWeeklyStats = async (
  ...
  try {
    ...
    // query lấy transaction trong 7 ngày gần nhất
    ...
    // console.log("error fetching fetchWeeklyStats: ", err);
    return { success: false, msg: err.message };
  }
};
...
export const fetchMonthlyStats = async (
  ...
  try {
    ...
    return {
      success: false,
      msg: "Lỗi không lấy được giao dịch theo tháng",
    };
  }
};
...
export const fetchYearlyStats = async (uid: string): Promise<ResponseType> => {
  try {
    ...
    // Reformat yearlyData for the bar chart with income and expense entries
    ...
    return {
      success: false,
      msg: "Lỗi không lấy được giao dịch theo năm",
    };
  }
};